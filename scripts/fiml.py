...
X, Y, c = data_gaussian_diag(n_samples=N_SAMPLES, sep_level=.9, random_state=42, nan_ratio=.00)
...
m = StepMix(n_steps=1, n_components=3, measurement='bernoulli', structural=f'gaussian_{COV}', rel_tol=1e-5, n_init=10,
...
m = StepMix(n_steps=1, n_components=3, measurement='bernoulli_nan', structural=f'gaussian_{COV}_nan', rel_tol=1e-5, n_init=10,
        random_state=42, max_iter=100)
m.fit(X, Y)
